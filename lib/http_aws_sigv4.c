...
static inline char *find_date_hdr(struct Curl_easy *data, const char *sig_hdr)
{
  char *tmp = Curl_checkheaders(data, sig_hdr);
  ...
  *date_header = find_date_hdr(data, date_hdr_key);
  if(!*date_header) {
    ...
    *date_header = strdup(*date_header);
    ...
    while(*tmp == ' ' || *tmp == '\t')
      ++tmp;
    strncpy(timestamp, tmp, TIMESTAMP_SIZE);
  ...
  for(l = head; l; l = l->next) {
    ...
    if(Curl_dyn_add(canonical_headers, l->data) != CURLE_OK)
      goto fail;
    ...
    if(l != head) {
      if(Curl_dyn_add(signed_headers, ";") != CURLE_OK)
        goto fail;
    }
